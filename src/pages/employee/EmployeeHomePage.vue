<template>
  <ion-page><ion-header><ion-toolbar><ion-title>Good day, {{ firstName }}</ion-title></ion-toolbar></ion-header>
    <ion-content>
      <ion-refresher slot="fixed" @ion-refresh="refresh"><ion-refresher-content /></ion-refresher>
      <profile-header :name="fullName" :subtitle="`${employee?.employee_code || 'No employee code'} · ${department}`" :status="employee?.status || 'unknown'" />
...
import { getTodayEmployeeHomeData } from "@/services/attendanceService"; import { useAuthStore } from "@/stores/auth"; import { useToast } from "@/composables/useToast"; import type { Tables } from "@/types/database.types";
...
async function refresh(event: CustomEvent) { await load(); (event.target as HTMLIonRefresherElement).complete(); } onMounted(load);
</script>
